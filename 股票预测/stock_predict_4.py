...
def lstm(X, keep_prob):
    ...
    # 初始化state
    init_state = mutil_cell.zero_state(batch_size, dtype=tf.float32)
    # 运行构建好的LSTM
    output_rnn, final_states = tf.nn.dynamic_rnn(mutil_cell, input_rnn, initial_state=init_state, dtype=tf.float32)
    # 输出
