...
proxy_ip = codecs.open('proxy_ip.txt', 'w', settings.FILE_ENCODE)  # 新建一个储存有效IP的文档
...
def test(i):
    ...
    url = "http://stock.gtimg.cn/data/index.php?appn=detail&action=download&c=sh600030&d=20170808"  # 打算爬取的网址
    try:
        proxy_support = urllib.request.ProxyHandler({settings.PROXY_TYPE: proxys[i]})
